...
AddEventHandler('wasabi_backpack:openBackpack', function(identifier)
	if not registeredStashes[identifier] then
        ox_inventory:RegisterStash('bag_'..identifier, 'Çanta', Config.BackpackStorage.slots, Config.BackpackStorage.weight, false)
        registeredStashes[identifier] = true
    end
    CreateInventoryHooks('bag')
end)
...
lib.callback.register('wasabi_backpack:getNewIdentifier', function(source, slot)
	...
	ox_inventory:RegisterStash('bag_'..newId, 'Çanta', Config.BackpackStorage.slots, Config.BackpackStorage.weight, false)
